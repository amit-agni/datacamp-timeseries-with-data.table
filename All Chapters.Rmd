...
* Well done! Selecting columns with names stored in a vector is a powerful tool to know.  
...
> You just took the first step towards implementing a fast feature-engineering library! The fact that the := operator changes the data.table in place also greatly reduces the risk that you'll blow out your memory.
...
Overall a very basic and superficial course. No depth, nothing on rolling joins, handling of missing time periods, etc. Few new things I learnt as summarised below :

* use of get() to extract the values inside columns as objects and LHS of := inside bracets tells R to evaluate this object and use its value as a column name
    + someDT[, (new_col_name) := get(col_name) ^ 2 ] 
    + The new_col_name is a vector hence should be within brackets ()  
...
* Implemented the add_diff and add_growth_rate and correlation matrix functions
